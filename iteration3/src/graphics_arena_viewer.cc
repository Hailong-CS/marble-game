...
GraphicsArenaViewer::GraphicsArenaViewer
  (const struct arena_params* const params)
    : csci3081::GraphicsApp(params->x_dim + 200, params->y_dim, "Robot Simulation"),
  ...
  nanogui::ref<nanogui::Window> window = 
    gui->addWindow(Eigen::Vector2i(params->x_dim, 10), "Menu");
...
/* previous cod: print the mouse location on the window or the key pressed
...
void GraphicsArenaViewer::DrawAutobot(NVGcontext *ctx, const Autobot* const autobot) {
  // translate and rotate all graphics calls that follow so that they are
  // centered, at the position and heading for this autobot
  nvgSave(ctx);
  nvgTranslate(ctx, autobot->get_pos().x, autobot->get_pos().y);
  nvgRotate(ctx, autobot->get_heading_angle() * M_PI / 180.0);

  // autobot's circle
  nvgBeginPath(ctx);
  nvgCircle(ctx, 0.0, 0.0, autobot->get_radius());
  nvgFillColor(ctx, nvgRGBA(autobot->get_color().r,
                            autobot->get_color().g,
                            autobot->get_color().b,
                            255));
  nvgFill(ctx);
  ...
  // autobot id text label
  nvgSave(ctx);
  nvgRotate(ctx, M_PI / 2.0);
  nvgFillColor(ctx, nvgRGBA(0, 0, 0, 255));
  nvgText(ctx, 0.0, 10.0, autobot->name().c_str(), NULL);
  nvgRestore(ctx);
  nvgRestore(ctx);
}

void GraphicsArenaViewer::DrawRobotSensors(NVGcontext *ctx, 
  const Autobot* const autobot) {
  // translate and rotate all graphics calls that follow so that they are
  // centered at the position and heading for this robot
  nvgSave(ctx);
  nvgTranslate(ctx, autobot->get_pos().x, autobot->get_pos().y);
  ...
  // sensor cone outline
  nvgSave(ctx);
  nvgRotate(ctx, 0.5 * 2); // 2 is the snesor angle in pi
  nvgBeginPath(ctx);
  nvgMoveTo(ctx, 0.0, 0.0);
  nvgLineTo(ctx, 7 * autobot->get_radius(), 0.0);
  nvgArc(ctx, 0.0, 0.0, 7 * autobot->get_radius(), 0.0, -2, NVG_CCW);
  ...
  nvgMoveTo(ctx, 0.0, 0.0);
  nvgLineTo(ctx, 7 * autobot->get_radius(), 0.0);
  nvgArc(ctx, 0.0, 0.0, 7 * autobot->get_radius(), 0.0, -0.5 * 2, NVG_CCW);
  nvgLineTo(ctx, 0.0, 0.0);
...
void GraphicsArenaViewer::DrawUsingNanoVG(NVGcontext *ctx) {
  ...
  std::vector<Autobot*> autobots = arena_->autobots();
  for (size_t i = 0; i < autobots.size(); i++) {
    DrawRobot(ctx, autobots[i]);
    DrawRobotSensors(ctx, autobots[i]);
  }
  ...
  DrawRobot(ctx, arena_->home_base());
}
